...
<body class="min-h-screen bg-gradient-to-br from-green-50 to-green-100 font-poppins" x-data="{
    ...
    <main class="max-w-6xl mx-auto px-4 pb-8">
        ...
        @if($errors->any())
            <div x-data="{ show: true }" x-show="show"
                 class="bg-gradient-to-r from-red-500 to-red-600 text-white px-6 py-4 rounded-2xl shadow-lg mb-6">
                        ...
                        <ul class="space-y-1 text-sm">
                            @foreach($errors->all() as $error)
                                <li class="flex items-start gap-2">
                                    <i class="bi bi-dot text-xl"></i>
                                    <span>{{ $error }}</span>
                                </li>
                            @endforeach
                        </ul>
                    </div>
                    <button @click="show = false" class="text-white hover:bg-white hover:bg-opacity-20 rounded-lg p-2 transition-all flex-shrink-0">
                        <i class="bi bi-x-lg"></i>
                    </button>
                </div>
            </div>
